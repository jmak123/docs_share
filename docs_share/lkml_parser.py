    ...
    def get_desc(self):
        ...
        if "views" in self.content:  ## only read lkml views
            for v in self.content["views"]:
                if (
                    "dimensions" in v
                ):  ## only read dimensions because we target "sql:" fields
                    for d in v["dimensions"]:
                        if (
                            "description" in d and "sql" in d and "sql_table_name" in v
                        ):  ## check if key identifiers are all there otherwise skip
                            self.desc.append(
                                {
                                    "relation": v["sql_table_name"].replace(
                                        LKML_TABLE_PREFIX, ""
                                    ),
